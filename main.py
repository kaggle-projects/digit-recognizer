...
from utensil.random_search import RandomizedConfig, ExponentialBetweenParam, RandomizedChoices, UniformBetweenParam, RandomizedDispatcher
...
@dataclass
class LinearConfig(RandomizedConfig):
    pass
...
linear_config_temp = LinearConfig()
...
svm_config_temp = SvmConfig(
    ...
    kernel_config=RandomizedDispatcher('kernel', {
        KernelChoices.LINEAR: linear_config_temp,
...
x, y = get_xy('data/train.csv', 'data/test.csv')
...
for model_id in range(10000):
    ...
    if isinstance(svm_config.kernel_config, LinearConfig):
        pass
    elif isinstance(svm_config.kernel_config, RbfConfig):
    ...
    if best_model is None or best_model[0] < score:
        ...
        submit_path = os.path.join('submit', f'submit_{int(np.round(score * 1e5))}_{model_id:04d}.csv')
        pd.DataFrame(te_xpy, columns=['ImageId', 'Label']).to_csv(submit_path, index=False)

    logger.info(f'{model_id=}: current best model_id={best_model[2]}, score={best_model[0]}, config={svm_config.to_plain_dict()}')
